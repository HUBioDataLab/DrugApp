...
#Data:
df=pd.read_csv(r"..\datasets\prospective_analysis_datasets\drug_indication_feature_vectors\{0}_drug_indication_feature_vectors.csv".format(sys.argv[1]),sep=',')
...
df_test=pd.read_csv(r"..\datasets\prospective_analysis_datasets\prospective_feature_vectors\{0}_prospective_feature_vectors.csv".format(sys.argv[1]),sep=',')
...
#Prediction scores (the probability of approval of a drug higher than 0.5):
df_concat=df_concat.loc[df_concat.Prob_approval >0.5]


#Save:
df_concat.to_csv("results_prospective_analysis_{0}.csv".format(sys.argv[1]),index=None)
